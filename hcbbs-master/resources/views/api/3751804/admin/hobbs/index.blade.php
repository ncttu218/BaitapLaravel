    ...
    <div class="row" style="background: white;">
        <form action="{{ $urlAction }}" method="post">
            ...
            <div class="col-sm-12" style="margin: 10px 0;">
                @include('api.common.admin.hobbs.pagination')
            </div>
            @foreach($blogs as $row)
                ...
                <div class="col-sm-12" style="border: 1px solid rgb(0, 0, 102);margin: 10px 0">
                    <div class="row">
                        ...
                        @if(($hansha_code === '3751804') && ($user_level === 5))
                            <div class="col-sm-12" style="text-align: right;">掲載処理&nbsp;
                                <label><input type="radio" name="{{ $row->number}}[published]" value="applying" {{ $row->published == 'applying' ? 'checked': null }}>掲載OK</label>&nbsp;&nbsp;
                                <label><input type="radio" name="{{ $row->number}}[published]" value="NG" {{ $row->published == 'NG' ? 'checked': null }}>掲載不可</label>&nbsp;&nbsp;
                                <label><input type="radio" name="{{ $row->number}}[published]" value="" {{ $row->published == 'OFF' ? 'checked': null }}>掲載待ち</label>
                            </div>
                        @else
                            <div class="col-sm-12" style="text-align: right;">掲載処理&nbsp;
                                <label><input type="radio" name="{{ $row->number}}[published]" value="ON" {{ $row->published == 'ON' ? 'checked': null }}>掲載OK</label>&nbsp;&nbsp;
                                <label><input type="radio" name="{{ $row->number}}[published]" value="NG" {{ $row->published == 'NG' ? 'checked': null }}>掲載不可</label>&nbsp;&nbsp;
                                <label><input type="radio" name="{{ $row->number}}[published]" value="" {{ ($row->published == 'OFF' || $row->published == 'applying') ? 'checked': null }}>掲載待ち</label>
                            </div>
                        @endif

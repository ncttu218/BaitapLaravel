...
<div class="p-entrylist-navigation">
    <div class="p-entrylist-pager">
        <div class="p-entrylist-pager__item"<?php echo $before_hid;?>>
            <a href="{{ $blogs->url($before) . $paramsStr }}" class="p-entrylist-pager__prev">前</a>
        </div>
        <?php
        for ($p = $currentPage; $p <= $page_end; $p++){
            ...
            <div class="p-entrylist-pager__item">
                <a href="{{ $blogs->url($p) . $paramsStr }}" class="p-entrylist-pager__button <?php echo $current;?>"><?php echo $p;?></a>
            </div>
        <?php } ?>
        <div class="p-entrylist-pager__item"<?php echo $next_hid;?>>
            <a href="{{ $blogs->url($next) . $paramsStr }}" class="p-entrylist-pager__next">次</a>
        </div>
    </div>

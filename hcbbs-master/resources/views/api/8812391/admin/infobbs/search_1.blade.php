        ...
        <form action="{{ $urlAction }}" method="post">
            ...
            @if ($blogs->count() > 0)
                ...
                @foreach($blogs as $row)
                    ...
                    // リリースのステータス
                    $editflag_release = ($row->editflag == 'release' || $row->editflag == 'honsya_tantoo' ) ? ' checked="checked"' : '';
                    // ドラフトのステータス
                    ...
                    switch ($row->published) {
                        ...
                        case 'OFF': $release_honsya = '◇掲載処理待ち';
                            break;
                        case 'applying': $release_honsya = '◇本社管理処理待ち';
                            break;
                    }
                    $num++;
                    ?>
                            ...
                            <div class="c-control-container p-entrylist-box__control">
                                ...
                                <div class="c-control-container__input">
                                    <div class="c-control-edit">
                                        <?php if($row->editflag == 'draft' || $row->editflag == 'release' || $row->editflag =='honsya_tantoo' || $row->editflag == ''){ //本社承認機能アリの場合（申請方式） ?>
                                        <dl class="c-control-edit__item">
                                            <dt class="c-control-edit__head">公開設定</dt>
                                            <dd class="c-control-edit__body">
                                                <table>
                                                    @if ($row->published === 'OFF' || $row->published === 'applying' || $row->published === 'NG')
                                                        <tr>

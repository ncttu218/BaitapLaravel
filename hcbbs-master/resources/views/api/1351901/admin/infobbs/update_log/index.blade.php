...
<table width="1200">
    <tr valign="bottom">
        ...
        <td align="center" id="top">
            ...
            ?>
            <font style="font-size:20; font-weight:bold;">　{{ $hanshaName }}　-情報掲示板 更新状況-　</font>
        </td>
        <td align="right">
...
<table width="1200">
    <tr>
        ...
        <td align="right">
            ...
            <a target="_blank" href="https://cgi2-aws.hondanet.co.jp/cgi/admin/bbs_count.cgi?id={{ $hanshaCode }}/infobbs">（旧）各店舗別集計ツールへ</a>
        </td>
    </tr>
</table>
    ...
    <tr>
        <td bgcolor="#00008B">
            <table width="1200">
                ...
                @foreach ($shopGroup as $group)
                <tr bgcolor="#CCEEFF" id="">
                    <td colspan="{{ $lastDay + 3 }}">
                        <table width="100%">
                            <tbody>
                                <tr>
                                    <td style="font-weight:bold;" nowrap="">第１営業部</td>
                ...
                @foreach ($group as $shopCode)
                <?php
                $shopName = $shopList[$shopCode];
                // 選択した月のトータル
                $shopLast = $shopLastArr[$shopCode] ?? 0;
                // 今月のトータル
                $thisMonthShopLast = $thisMonthTotalByShopArr[$shopCode] ?? 0;
                $status = '';
                if ($thisMonthShopLast == 0) {
                ...
                <tr align="center" bgcolor="#ffffff">
                    <td{{ $status }} align="left" nowrap>
                        <?php
                        // ブログ公開API
                        $url = 'https://www.hondacars-saitama.co.jp/home/sr.html?shop=' . $shopCode;
                        ?>

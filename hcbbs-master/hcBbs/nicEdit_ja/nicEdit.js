...
var nicEditorInstance = bkClass.extend({
	...
	textToHtml : function(thisText) {
		return (!this.ne.options.convertToText) ? thisText : thisText.replace(/(?:\r\n|\n|\r)(?=<(?:table|caption|thead|tbody|tfoot|tr|th|td|ol|ul|li|dl|dt|dd|div|p|address|h\d|blockquote|fieldset|form|pre|hr)(?!\w)[^>]*>)/gi,'').replace(/(<\/(?:table|caption|thead|tbody|tfoot|tr|th|td|ol|ul|li|dl|dt|dd|div|p|address|h\d|blockquote|fieldset|form|pre|hr)>)(?:\r\n|\n|\r)/gi,"$1").replace(/(\n\n|\r\r|\r\n\r\n)/g,"</p><p>").replace(/(?:\r\n|\n|\r)/g,"<br />").replace(/^(.*<\/p><p>)/,"<p>$1").replace(/(<\/p><p>.*)$/,"$1</p>");
	},
...
var nicEditorIFrameInstance = nicEditorInstance.extend({
	...
	initFrame : function() {
		var initialContent;
		if (this.tempContent) {
			initialContent = this.tempContent;
...
var nicEditorFontFamilySelect = nicEditorSelect.extend({
	sel : {
		"'Hiragino Kaku Gothic Pro','ヒラギノ角ゴ Pro W3','メイリオ','Meiryo','MS Pゴシック',Osaka,sans-serif" : 'ゴシック',
		"'Hiragino Mincho Pro','ヒラギノ明朝 Pro W3','ＭＳ Ｐ明朝','MS PMincho',serif" : '明朝',
...
var nicImageButton = nicEditorAdvancedButton.extend({	
	addPane : function() {
		...
		this.addForm({
			...
			'align' : {type : 'select', txt : '位置', options : {none : 'インライン','left' : '左', 'right' : '右'}}
		},this.im);
	},
...
var nicXHTML = bkClass.extend({
	...
	noChild : ['br','link','meta','base','textarea','input','option','img','object','embed','param','script','style','a','video','audio','source'],
			...
			case 1:
				...
				if(r) {
					for(var i=0;i<nAttributes.length;i++) {
						...
						var attributeValue = (attr.nodeValue != null) ? this.escapeHTML(attr.nodeValue) : (attributeName == "style" && n.style.cssText != null) ? n.style.cssText.replace(/[\w\-]+:/g,function(s){return s.toLowerCase()}) : (attributeName == "class" && n.className != null) ? n.className : null;
						...
						switch(attributeName) {
							...
							break;
							case 'size':
								cssTxt = cssTxt.replace(/font-size:\s*[^;]+;?/g,"");
								if (!attributeValue || attributeValue > 6) attributeValue = parseInt(attributeValue/16);
								cssTxt += "font-size:"+this.sizes[attributeValue]+';';
								attributeValue = "";
							break;
							case 'face':
								cssTxt = cssTxt.replace(/font-family:\s*[^;]+;?/g,"");
								cssTxt += "font-family:"+attributeValue+';';
								attributeValue = "";
							break;
							case 'color':
								cssTxt = cssTxt.replace(/(^|;)\s*color:\s*[^;]+/g,"");
								cssTxt += "color:"+attributeValue+';';
								attributeValue = "";
							break;
							case 'bgcolor':
								cssTxt = cssTxt.replace(/background-color:\s*[^;]+;?/g,"");
								cssTxt += "background-color:"+attributeValue+';';
								attributeValue = "";
							break;
							case 'align':
								if (nName == "img") {
									cssTxt = cssTxt.replace(/float:\s*[^;]+;?/g,"");
									cssTxt += "float:"+attributeValue+';';
								} else {
									cssTxt = cssTxt.replace(/text-align:\s*[^;]+;?/g,"");

...
trait StaffbbsApiChecker {
     ...
     */
    protected function loadPageHtml($page) {
        $method = 'GET';
        $route = "/api/staffbbs/{$page}?"
     ...
     * @param string $method メソッド名
     */
    protected function loadPageJson($page) {
        $method = 'GET';
        $route = "/api/staffbbs/{$page}?show_type=json&"
            . "page_num={$this->pageNum}&"
            . "hansha_code={$this->hanshaCode}";
        $this->response = $this->call($method, $route);
    }

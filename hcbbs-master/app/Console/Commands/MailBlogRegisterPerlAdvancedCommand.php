 ...
 * @author ahmad
 */
class MailBlogRegisterPerlAdvancedCommand extends Command {
    ...
    public function handle() {
        try {
            ...
            $errorMsg = $file . ': ' . $ex->getMessage();
            $this->writeLog($errorMsg, $logFile);
    ...
    public function runCore() {
        ...
        foreach ($files as $tempMailFile) {
            ...
            $emailData = $mailParser->parseEmailFile($tempMailFile);
            // データ取得が失敗した場合
            ...
            // メール種類の確認
            if ($fromObj->mailbox . '@' . $fromObj->host == self::SERVER_NOTIFICATION_EMAIL) {
                // 種類　→　メール登録
                $emailAddr = $message->getContents();
                ...
                $pattern = self::EMAIL_REGISTRATION_SUBJECT .
                        ...
                        . '\.?(data[0-9]{6})?$/';
                if (!preg_match('/^' . $pattern, $mailSubject, $match)) {
                    // ファイルを削除して、終了
                    $this->trashFile($tempMailFile);
                    $this->closingMessage();
                    return;
                ...
                $this->closingMessage();
                return;
            } else if ($toObj->host == self::EMAIL_HOST &&
                    preg_match('/^([0-9]+?)\.(infobbs|staff)/', $toObj->mailbox, $match)) {
                // 種類　→　転送メール投稿
                // 販社コード
                $hanshaCode = $match[1];
                // システム名
            ...
            $hasAttachment = false;
            // メールの内容ボディー
            $mailParser->messageTempDirname = str_replace('.eml', '', $tempMailFile);
            $mailParser->baseName = $baseName;
            $mailParser->month = $this->month;
            $mailContent = $mailParser->parseMailContent(
                $message,
                // コールバックの定義
                function ($text, $files)
                    use(&$hasAttachment, $systemName, $hanshaCode, $baseName)
                {
                    // 画像ファイルのパーシングが失敗した場合、画像 = 0
                    $hasAttachment = count($files) > 0;
                    ...
                    foreach ($files as $attachmentId => $file) {
                        if ($this->uploadServer == self::UPLOAD_SERVER_EXTERNAL) {
                            ...
                            // 内部サーバーに保存
                            $url = '';
                            $this->copyImageToInternalServer(
                                $file,
                                $postInfo,
                                $url
                            );
                        ...
                        // 本文を添付画像に置換
                        $pattern = '/(<img[\w\W]+?src=")cid:' . $attachmentId . '(")/';
                        $text = preg_replace($pattern, "$1{$url}$2", $text);
                    }
                    
                    // 本文を添付画像に置換
                    /*return $this->replaceContentWithAttachmentImages(
                        $text,
                        $urls
                    );*/
                    return $text;
            ...
            if ($hasAttachment) {
                $tempDirName = $mailParser->messageTempDirname;
                $regFileName = "{$monthDirName}/{$baseName}.eml";
            }
     ...
     */
    private function copyImageToInternalServer($path, $postInfo, &$url) {
        // 販社コード
     ...
     * @param array $urls 画像ファイルのリスト
     * @param string $postInfo 記事情報
     *  ・hanshaCode 販社コード
     *  ・systemName システム名
     * @return string
     */
    private function replaceContentWithAttachmentImages($mailContent, $urls) {
        $mailContent = preg_replace_callback(
            '/(<img[\w\W]+?src=")cid:(.*?)(")/',
            function($match) use($urls) 
            {
            // 添付ID
            $attachmentId = $match[2];
            $url = $urls[$attachmentId];

 ...
 *
 */
class InfobbsController extends Controller
{
    ...
    protected $notificationTypes = [
        InfobbsCoreController::NOTIFICATION_POST_REGISTERED,
    ];
    ...
    public function __construct() {
        $this->controller = "Infobbs\InfobbsController";
        // ユーザ情報
        $this->loginAccountObj = SessionUtil::getUser();
        // 販社コード
        $this->hanshaCode = $this->loginAccountObj !== null ?
            $this->loginAccountObj->getHanshaCode() : '';
    ...
    public function getIndex(){
        ...
        return view('infobbs.search_' . $this->published_mode, $list) // 掲載設定機能で親テンプレート切り替え
    ...
    public function postIndex(){
        ...
        return view('infobbs.complete')
            ->with('urlAction', action_auto("Infobbs\InfobbsController" . '@getIndex') . '?shop=' . $shop)
            ->with('msg','データを編集しました。');
    }
    ...
    public function getUpload(){
        ...
        return view('infobbs.upload')->with('data', $data)
            ->with('template', $template)
            ->with('dataType', 'array')
    ...
    public $validateRules = [
        'comment'=>'required', // 内容
        'to_date' => 'nullable|after:from_date', // 掲載日
    ];
    // バリデーションのエラーメッセージ
    public $validateMessages = [
        "required" => ":attributeは必須項目です。",
        "to_date.after" => "掲載期間 終了日は掲載日よりも後の日付を入力してください。",
    ];
...
//        );
        ...
        if(isset($req['delete4'])){
            $data['file4'] = '';
        }
        if(isset($req['delete5'])){
            $data['file5'] = '';
        }
        if(isset($req['delete6'])){
            $data['file6'] = '';
        }
        // セッションに保存
        SessionUtil::putData($data);
...
//        }
    ...
    public function getConfirm(){
        ...
        return view('infobbs.confirm')->with('data', $data)
            ->with('template', $template)
            ->with('dataType', 'array')
            ->with('shopList', $shopList)  // 店舗セレクトボックス
    ...
    public function postConfirm(){
        ...
        if(isset($req['register'])){
            ...
            $urlAction = action_auto("Infobbs\InfobbsController" . '@getIndex') . '?shop=' . $data['shop'];

            return view('infobbs.complete', $data)->with('msg','データを登録しました。')
    ...
    public function postDelete(){
        ...
        if(isset($req['del'])){
            ...
            // テーブル削除
            $infoBbsInstance = Infobbs::createNewInstance( $this->hanshaCode );
            $infoBbsInstance->where('number', $data['number'])
                    ->delete();
           return view('infobbs.complete')->with('msg','データを削除しました。')
                ->with('urlAction',action_auto("Infobbs\InfobbsController" . '@getIndex'));
        }
    }

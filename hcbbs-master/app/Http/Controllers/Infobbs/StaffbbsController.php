...
class StaffbbsController extends Controller
{
    ...
    public function __construct()
    {
        ...
        SessionUtil::putTableName('tb_' . $this->loginAccountObj->getHanshaCode() . '_staff');
        ...
        $this->staff_comment = $para['staff_comment'];

        $this->controller = "Infobbs\StaffbbsController";
        // テンプレートNo
    ...
    public function getIndex()
    {
        ...
        return view('infobbs.staff.admin', $list) // 掲載設定機能で親テンプレート切り替え
    ...
    public function postIndex(){
        ...
        return view('infobbs.complete')
            ->with('urlAction', action_auto("Infobbs\StaffbbsController" . '@getIndex') . '?shop=' . $shopCode)
            ->with('msg','データを編集しました。');
    }
    ...
    public function getEditProfile(){
        ...
        return view('infobbs.staff.profile_edit')->with('data', $data)
            ->with('template', $template)
            ->with('dataType', 'array')
            ->with('mode', SessionUtil::getMode())          // 新規 or 編集
            ->with('upflag', SessionUtil::getUpflag())      // 更新フラグ
            ->with('shopList', $shopList)  // 店舗セレクトボックス
//            ->with('urlImageTmp',action_auto("Staffbbs\ImageTmpController" . '@getIndex')) // // 画像アップロード機能（本文）
    ...
    public function postEditProfile( StaffRequest $req ){
        ...
        $data['shop'] = $req['shop'];
        if (isset($req['photo_del'])) {
            $data['photo_del'] = $req['photo_del'];
        }
    ...
    public function getConfirm(){
        ...
        return view('infobbs.staff.profile_edit_confirm')->with('data', $data)
            ->with('template', $template)
            ->with('dataType', 'array')
            ->with('shopList', $shopList)  // 店舗セレクトボックス
    ...
    public function postConfirm()
    {
        ...
        if(isset($req['register'])){
            ...
            $shopCode = SessionUtil::getShop();
            $urlAction = action_auto("Infobbs\StaffbbsController" . '@getIndex') . '?shop=' . $shopCode;
            if (SessionUtil::getHomeName() == 'LIST') {
                $urlAction = action_auto("Infobbs\StaffbbsController" . '@getList');
            }

            return view('infobbs.staff.complete', $data)->with('msg','データを登録しました。')
    ...
    public function getList()
    {
        ...
        return view('infobbs.staff.list', $list) // 掲載設定機能で親テンプレート切り替え
            ->with('template', $template)
            ->with('dataType', 'object')
            ->with('shopList', $shopList) // 店舗セレクトボックス
            ->with('shopName', $shopName) // 店舗セレクトselected
    ...
    public function getSingleEdit(){
        ...
        return view('infobbs.staff.single_edit')->with('data', $data)
            ->with('dataType', 'array')
...
*/
    ...
    public function getBlogList(){
        ...
        // コメント取得
        $comment = array();
        if($this->staff_comment == '1'){
            $comment = $this->makeComment($list['blogs']);
        }
        return view('infobbs.staff.blog_list', $list) // 掲載設定機能で親テンプレート切り替え
            ->with('template', $template)
            ->with('dataType', 'object')
            ->with('shopList', $shopList) // 店舗セレクトボックス
            ->with('shopName', $shopName) // 店舗セレクトselected
    ...
    public function postBlogList(){
        ...
        return view('infobbs.staff.complete')
            ->with('urlAction', action_auto("Infobbs\StaffbbsController" . '@getBlogList') . '?shop=' . $data['shop'])
            ->with('msg','データを編集しました。');
    }
    ...
    public function getBlogEdit()
    {
        ...
        return view('infobbs.staff.blog_edit')->with('data', $data)
            ->with('template', $template)
            ->with('dataType', 'array')
...
*/
    ...
    public function getConfirmBlog(){
        ...
        return view('infobbs.staff.blog_edit_confirm')->with('data', $data)
            ->with('template', $template)
            ->with('dataType', 'array')
            ->with('shopList', $shopList)  // 店舗セレクトボックス
            ->with('shopName', $shopName)
    ...
    public function postConfirmBlog(){
        ...
        if(isset($req['register'])){
            ...
            $urlAction = action_auto("Infobbs\StaffbbsController" . '@getBlogList');
    ...
    public function getDelete(){
        ...
        return view('infobbs.delete')->with('data', $data)
            ->with('name', SessionUtil::getName())
            ->with('urlAction',action_auto($this->controller . '@postDelete'));
    }
    ...
    public function postDelete(){
        ...
        if(isset($req['del'])){
            $data = SessionUtil::getData();
            // テーブル削除
            DB::table(SessionUtil::getTableName())->where('number', $data['number'])
                ->delete();
           return view('infobbs.complete')->with('msg','データを削除しました。')
                ->with('urlAction',action_auto("Staffbbs\StaffbbsController" . '@getIndex'));
        }
    }
    ...
    protected function makeComment($blog){
        ...
        $table_name = 'tb_'.$this->tableNo.'_staff_comment';

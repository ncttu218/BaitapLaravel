...
trait DesignRepository
{
    ...
    public function getEditDesign() {
        ...
        // URL
        $urlConfirmDesign = CodeUtil::getV2Url('Admin\AdminController@postConfirmDesign', $this->hanshaCode);
        // ビュー名
     ...
     * 背景デザインを確認する画面
     */
    public function postConfirmDesign() {
        // リクエスト
        $reqs = Request::all();
        // 画像を読み込む
        ...
        // URL
        $urlSaveDesign = CodeUtil::getV2Url('Admin\AdminController@postSaveDesign', $this->hanshaCode);
        $urlEditDesign = CodeUtil::getV2Url('Admin\AdminController@getEditDesign', $this->hanshaCode) . '?shop=' . $shopCode;
        // ビュー名
    ...
    public function postSaveDesign() {
        ...
        if ($draftDesignObj) {
            ...
            if ($trueDesignObj) {
                $data = [];
                foreach ($draftDesignObj->getAttributes() as $key => $value) {
                    $data[$key] = $value;
                }
                unset($data['id']);
                unset($data['shop']);

...
trait MessageRepository
{
    ...
    public function getIndex() {
        ...
        return view( 'api.common.admin.message.index', 
            compact(
                'title',
                'showData',
                'shopName'
    ...
    public function postIndex() {
        ...
        foreach ( $request as $key => $val ){
            ...
            if(strstr($key,'data')){
                if(isset($val['del'])){
                    // テーブル削除
                    $messageInstance = Message::createNewInstance( $this->hanshaCode );
                    $messageInstance = $messageInstance->where('number', $key);
                    if ($this->shopDesignation) {
                        $messageInstance = $messageInstance->where('shop', $request['shop']);
                    }
...
//        dd($setValue);
        ...
        // ビュー名
        $viewName = "api.{$this->hanshaCode}.admin.message.input";
        if (!view()->exists($viewName)) {
            $viewName = "api.common.admin.message.input";
        }
    ...
    public function postConfirm( MessageRequest $request ){
        ...
        // ビュー名
        $viewName = "api.{$this->hanshaCode}.admin.message.confirm";
        if (!view()->exists($viewName)) {
            $viewName = "api.common.admin.message.confirm";
        }
        
        // リンクの開き方コード
        $urlTargets = (new UrlTargetCodes)->getOptions();
    ...
    public function getThanks() {
        ...
        return view( 
            'api.common.admin.message.thanks'
        )
        ->with( 'title', "お知らせ 1行メッセージ" )
        ->with( 'hanshaCode', $this->hanshaCode )
        ->with( 'shopCode', $this->shopCode )
    ...
    public function getDetail( $id, $shopCode = null ){
        ...
        // ビュー名
        $viewName = "api.{$this->hanshaCode}.admin.message.detail";
        if (!view()->exists($viewName)) {
            $viewName = "api.common.admin.message.detail";
        }

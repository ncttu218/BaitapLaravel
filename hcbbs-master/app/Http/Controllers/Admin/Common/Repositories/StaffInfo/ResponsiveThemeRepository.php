...
namespace App\Http\Controllers\Admin\Common\Repositories\StaffInfo;
 ...
 * @author ahmad
 */
trait ResponsiveThemeRepository {
    ...
    public function getList() {
        ...
        // 並び順
        if ($this->hanshaCode == '1103901') { // 横浜
            $staffInfoObj = $staffInfoObj->orderBy( 'department' )
                    ->orderByRaw( 'grade::int asc' );
        } else {
            $staffInfoObj = $staffInfoObj->orderBy( 'regist' );
        }
        ...
        // ページネーションのロカールビューフラグ
        $viewContentName = 'api.' . $this->hanshaCode . '.admin.staffinfo.responsive.pagination';
        $hasLocalPaginationView = view()->exists($viewContentName);
        
        return view( 
                $viewName, 
                compact( 'list', 'departmentCodes', 'shopName', 'hasLocalView', 'hasLocalPaginationView' ) 
            )

...
class InfobbsController extends Controller
{
     ...
     * @var bool
     */
    private $isMultipleRecord = false;
    ...
    public function __construct()
    {
        ...
        if ($this->deviceType === 'pc' || $this->deviceType === 'lite') {
            $this->templateDir = 'api.' . $this->hanshaCode . '.infobbs.' . $this->deviceType;
        } else {
            $this->templateDir = 'api.' . $this->hanshaCode . '.infobbs.pc';
        }
    ...
    private function buildOrderBy( $query, $prefix = '', $dir = 'desc' ) {
        ...
        return $query->orderByRaw( "CASE WHEN {$prefix}from_date IS NULL "
                    . "THEN {$prefix}updated_at ELSE {$prefix}from_date END {$dir}" )
    ...
    public function getBlog()
    {
        ...
        // パラメーター設定
        $configPara = \config( 'original.para' );

        // 値が無いときはエラーを出力
        if( $this->hanshaCode === "" || !isset($configPara[$this->hanshaCode]) ){
            return "<p>表示エラー</p>";
        }
        ...
        if( $this->pageNum !== ""){
            ...
            // 1ページの表示件数
            $perPage = $configPara[$this->hanshaCode]['page_num'] ?? (int)$this->pageNum;
        }
    ...
    public function getLatestBlog() {
        ...
        if( $this->showType == "json" ){
            ...
            return view( $this->templateDir . '.latestBlog' . $viewStyle, $list )
                ->with( 'hanshaCode', $this->hanshaCode )
                ->with( 'templateDir', $this->templateDir );
        }
    }
    ...
    public function getNotice()
    {
        ...
        // 出力するデータ
        $url = '#';
        $title = '台風接近に伴う営業時間変更について';
        
        return view( $this->templateDir . '.notice', compact(
            'url',
            'title'
        ) )
            ->with( 'hanshaCode', $this->hanshaCode )
    ...
    private function getPagingData($res) {
        if ($this->isMultipleRecord) {
            ...
            // 次の記事
            $query = DB::table( $this->tableName );
            $next = $query->whereRaw("SUBSTRING(number, 5) > '{$res}'")
                    // 公開期間内のとき
                    ->whereRaw( '((from_date <= now() AND to_date >= now()) OR '
                        . '(from_date <= now() AND to_date IS NULL) OR '
                        . '(from_date IS NULL AND to_date >= now()) OR '
                    ...
                    // 公開設定
                    ->where( 'published', 'ON' );
            $next = $this->buildOrderBy( $next, '', 'asc' );
            $next = $next->first();
            $nextNumber = null;
            if ($next !== null) {
            ...
            // 前の記事
            $query = DB::table( $this->tableName );
            $prev = $query->whereRaw("SUBSTRING(number, 5) < '{$res}'")
                    // 公開期間内のとき
                    ->whereRaw( '((from_date <= now() AND to_date >= now()) OR '
                        . '(from_date <= now() AND to_date IS NULL) OR '
                        . '(from_date IS NULL AND to_date >= now()) OR '
                    ...
                    // 公開設定
                    ->where( 'published', 'ON' );
            $prev = $this->buildOrderBy( $prev );
            $prev = $prev->first();
            $prevNumber = null;
            if ($prev !== null) {
                $prevNumber = str_replace('data', '', $prev->number);
            }
